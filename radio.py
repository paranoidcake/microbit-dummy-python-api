# Text taken from https://microbit-micropython.readthedocs.io/en/latest/radio.html
...
def config(**kwargs):
    ...
    The `channel` (default=7) can be an integer value from 0 to 83 (inclusive) that defines an arbitrary “channel” to which the radio is tuned.
    ...
    The `power` (default=6) is an integer value from 0 to 7 (inclusive) to indicate the strength of signal used when broadcasting a message.
        The higher the value the stronger the signal, but the more power is consumed by the device.
        The numbering translates to positions in the following list of dBm (decibel milliwatt) values: -30, -20, -16, -12, -8, -4, 0, 4.

    The `address` (default=0x75626974) is an arbitrary name, expressed as a 32-bit address, that’s used to filter incoming packets at the hardware level, keeping only those that match the address you set.
        The default used by other micro:bit related platforms is the default setting used here.

    The `group` (default=0) is an 8-bit value (0-255) used with the `address` when filtering messages.
...
def receive_bytes_into(buffer: Any) -> int:
...
def send(message: str):
    ...
    This is the equivalent of `send_bytes(bytes(message, 'utf8'))` but with `b'\x01\x00\x01'` prepended to the front (to make it compatible with other platforms that target the micro:bit).
    """

def recieve() -> str:
    """Works in exactly the same way as receive_bytes but returns whatever was sent.

    Currently, it’s equivalent to `str(receive_bytes(), 'utf8')` but with a check that the the first three bytes are `b'\x01\x00\x01'` (to make it compatible with other platforms that may target the micro:bit).
    It strips the prepended bytes before converting to a string.

    A `ValueError` exception is raised if conversion to string fails.
    """

def recieve_full() -> Tuple[bytes, int, int]:
